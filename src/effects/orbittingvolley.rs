...
struct OVChar {
    final_y: usize, final_x: usize, cur_y: f64, cur_x: f64,
    original_ch: char, final_color: Rgb, progress: f64, speed: f64,
    active: bool, done: bool, start_y: f64, start_x: f64,
}
...
pub struct OrbittingVolleyEffect {
    ...
    volley_delay: usize, delay_count: usize, volley_size: usize,
...
impl OrbittingVolleyEffect {
    pub fn new(grid: &Grid) -> Self {
        ...
        let final_gradient = Gradient::new(
            &[Rgb::from_hex("FFA15C"), Rgb::from_hex("44D492")], 12,
        );
        let cy = height as f64 / 2.0; let cx = width as f64 / 2.0;
        let launchers = [
            (0.0, cx), (cy, width as f64 - 1.0),
            (height as f64 - 1.0, cx), (cy, 0.0),
        ];
        ...
        for (i, &(y, x, _)) in indexed.iter().enumerate() {
            ...
            pending.push(i);
            chars.push(OVChar {
                final_y: y, final_x: x, cur_y: 0.0, cur_x: 0.0,
                original_ch: grid.cells[y][x].ch, final_color: fc,
                progress: 0.0, speed: 0.0, active: false, done: false,
                start_y: 0.0, start_x: 0.0,
            });
        }

        let volley_size = ((width * height) as f64 * 0.03).max(1.0) as usize;
        OrbittingVolleyEffect {
            chars, pending, launchers, launcher_angles: [0.0; 4],
            volley_delay: 30 * dm, delay_count: 0, volley_size,
            dm, width, height,
        }
    }
    ...
    pub fn tick(&mut self, grid: &mut Grid) -> bool {
            ...
            if self.delay_count >= self.volley_delay {
                ...
                for li in 0..4 {
                    ...
                    for _ in 0..self.volley_size.min(self.pending.len()) {
                        ...
                        ch.active = true;
                        ch.start_y = ly; ch.start_x = lx;
                        ch.cur_y = ly; ch.cur_x = lx;
                        let dist = ((ch.final_y as f64 - ly).powi(2) + (ch.final_x as f64 - lx).powi(2)).sqrt().max(1.0);
        ...
        for ch in &self.chars {
            ...
            cell.fg = Some(Rgb::lerp(Rgb::from_hex("FFA15C"), ch.final_color, ch.progress).to_crossterm());
        }

        // Render launchers
        for i in 0..4 {
            let (ly, lx) = self.launchers[i];
            let ry = ly.round() as usize; let rx = lx.round() as usize;
            if ry < self.height && rx < self.width {
                let cell = &mut grid.cells[ry][rx];
                cell.visible = true; cell.ch = '█';
