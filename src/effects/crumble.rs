...
enum CrumblePhase { Weakening, Falling, Vacuuming, Resetting, Done }

struct CrumbleChar {
    final_y: usize, final_x: usize, cur_y: f64, original_ch: char,
    final_color: Rgb, phase: CrumblePhase, frame: usize, progress: f64, speed: f64,
    fall_delay: usize, delay_count: usize,
}
...
impl CrumbleEffect {
    pub fn new(grid: &Grid) -> Self {
        ...
        let final_gradient = Gradient::new(
            &[Rgb::from_hex("5CE1FF"), Rgb::from_hex("FF8C00")], 12,
        );
        let mut rng = rand::thread_rng();
        let mut chars = Vec::with_capacity(width * height);
        for y in 0..height {
            for x in 0..width {
                let fc = final_gradient.color_at_coord(y, x, height, width, GradientDirection::Diagonal);
                chars.push(CrumbleChar {
                    final_y: y, final_x: x, cur_y: y as f64,
                    original_ch: grid.cells[y][x].ch, final_color: fc,
                    phase: CrumblePhase::Weakening, frame: 0, progress: 0.0,
        ...
        CrumbleEffect { chars, dm, width, height }
    }
    ...
    pub fn tick(&mut self, grid: &mut Grid) -> bool {
            ...
            match ch.phase {
                ...
                CrumblePhase::Falling => {
                    ...
                    if ch.progress >= 1.0 {
                        ...
                        ch.cur_y = ch.final_y as f64 + (self.height as f64 - ch.final_y as f64) * eased;
                    }
                    all_done = false;
                }
                ...
                CrumblePhase::Resetting => {
                    ...
                    if ch.phase != CrumblePhase::Done { all_done = false; }
                }
                CrumblePhase::Done => {}
            }
        }
        ...
        for ch in &self.chars {
            ...
            if ry < 0 { continue; }
            let ry = ry as usize;
            if ry >= self.height || ch.final_x >= self.width { continue; }
